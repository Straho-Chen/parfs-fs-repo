...
static inline void
nova_rebuild_file_time_and_size(struct super_block *sb,
				struct nova_inode_rebuild *reb, u32 mtime,
				u32 ctime, u64 size)
{
				       ...
				       struct nova_file_write_entry *entryc)
{
	...
	nova_reset_csum_parity_range(sb, sih, entry, entryc->pgoff, end_pgoff,
				     0, 1);
				       ...
				       struct nova_mmap_entry *entryc)
{
	...
	end_pgoff = entryc->pgoff + entryc->num_pages;
	nova_reset_csum_parity_range(sb, sih, NULL, entryc->pgoff, end_pgoff, 0,
				     0);
	...
	struct nova_file_write_entry *entryc)
{
	...
	if (entryc->trans_id >= reb->trans_id) {
		nova_rebuild_file_time_and_size(sb, reb, entryc->mtime,
						entryc->mtime, entryc->size);
	...
	while (curr_p != sih->log_tail) {
		...
		case SET_ATTR:
			...
			if (attr_entry->trans_id >= reb->trans_id) {
				nova_rebuild_file_time_and_size(
					sb, reb, attr_entry->mtime,
					attr_entry->ctime, attr_entry->size);
		...
		case MMAP_WRITE:
			...
			nova_reset_mmap_csum_parity(sb, sih, mmap_entry,
						    MMENTRY(entryc));
...
static inline void nova_rebuild_dir_time_and_size(
	struct super_block *sb, struct nova_inode_rebuild *reb,
	struct nova_dentry *entry, struct nova_dentry *entryc)
{
	if (!entry || !reb)
		return;
					 ...
					 struct nova_dentry *entryc)
{
	...
	return nova_insert_dir_tree(sb, sih, entryc->name, entryc->name_len,
				    entry);
}
					    ...
					    struct nova_dentry *entry)
{
	...
	nova_remove_dir_tree(sb, sih, entry->name, entry->name_len, 1, NULL);
	return 0;
}
...
static int nova_rebuild_handle_dentry(struct super_block *sb,
				      ...
				      struct nova_inode_rebuild *reb,
				      struct nova_dentry *entry,
				      struct nova_dentry *entryc, u64 curr_p)
{
	int ret = 0;
...
int nova_rebuild_dir_inode_tree(struct super_block *sb, struct nova_inode *pi,
				u64 pi_addr, struct nova_inode_info_header *sih)
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	...
	while (curr_p != sih->log_tail) {
		...
		case DIR_LOG:
			...
			ret = nova_rebuild_handle_dentry(
				sb, sih, reb, entry, DENTRY(entryc), curr_p);
			if (ret)
				goto out;
...
int nova_restore_snapshot_table(struct super_block *sb, int just_init)
{
	...
	while (curr_p != sih->log_tail) {
		...
		case SNAPSHOT_INFO:
			...
			ret = nova_restore_snapshot_entry(sb, entry, curr_p,
							  just_init);
			if (ret) {
