...
{
	return single_open(file, nova_seq_show_allocator, pde_data(inode));
}
...
{
	return single_open(file, nova_seq_show_snapshots, pde_data(inode));
}
...
static int nova_seq_test_perf_show(struct seq_file *seq, void *v)
{
	seq_printf(
		seq,
		"Echo function:poolmb:size:disks to test function performance working on size of data.\n"
...
ssize_t nova_seq_gc(struct file *filp, const char __user *buf, size_t len,
		    loff_t *ppos)
{
...
void nova_sysfs_init(struct super_block *sb)
{
	...
	if (sbi->s_proc) {
		...
		proc_create_data("gc", 0444, sbi->s_proc, &nova_seq_gc_fops,
				 sb);
	}
}
