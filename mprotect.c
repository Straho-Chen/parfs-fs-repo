				   ...
				   unsigned long num_pages)
{
	...
	for (i = 0; i < num_pages; i++) {
		...
		pentry = radix_tree_lookup_slot(&mapping->i_pages, curr_pgoff);
		if (pentry) {
	...
	struct vm_area_struct *vma, struct nova_file_write_entry *entry_data)
{
	...
	ret = nova_update_dax_mapping(sb, sih, vma, entry_data, start_pgoff,
				      num_pages);
	if (ret) {
	...
	ret = nova_update_entry_pfn(sb, sih, vma, entry_data, start_pgoff,
				    num_pages);
	if (ret)
...
int nova_mmap_to_new_blocks(struct vm_area_struct *vma, unsigned long address)
{
	...
	while (start_blk < end_blk) {
		...
		copied = bytes -
			 memcpy_to_pmem_nocache(to_kmem, from_kmem, bytes);
...
static inline bool pgoff_in_vma(struct vm_area_struct *vma, unsigned long pgoff)
{
	unsigned long num_pages;
