						     ...
						     unsigned int num_free)
{
	...
	if (invalid) {
		u64 addr = nova_get_addr_off(NOVA_SB(sb), entry, 1);
...
static inline int
nova_invalidate_write_entry(struct super_block *sb,
		     ...
		     pgoff_t pgoff)
{
	...
	nr_entries =
		radix_tree_gang_lookup(&sih->tree, (void **)entries, pgoff, 1);
	if (nr_entries == 1)
		entry = entries[0];
				      ...
				      struct nova_log_entry_info *entry_info)
{
	...
	entry->ctime = cpu_to_le32(inode_get_ctime_sec(inode));
...
static void
nova_update_link_change_entry(struct inode *inode,
...
static int nova_update_old_dentry(struct super_block *sb, struct inode *dir,
				  ...
				  struct nova_log_entry_info *entry_info)
{
	...
	addr = nova_get_addr_off(NOVA_SB(sb), dentry, 1);
	nova_inc_page_invalid_entries(sb, addr);
...
static int nova_update_new_dentry(struct super_block *sb, struct inode *dir,
				 ...
				 struct nova_log_entry_info *entry_info)
{
	...
	case FILE_WRITE:
		if (entry_info->inplace)
			...
			// small data, use memcpy+flush
			memcpy(entry, entry_info->data,
			       sizeof(struct nova_file_write_entry));
			nova_flush_buffer(
				entry, sizeof(struct nova_file_write_entry), 0);
		}
		break;
	case DIR_LOG:
		if (entry_info->inplace)
...
static int nova_append_log_entry(struct super_block *sb, struct nova_inode *pi,
				 ...
				 struct nova_log_entry_info *entry_info)
{
	...
	curr_p = nova_get_append_head(sb, pi, pic, sih, tail, size, MAIN_LOG, 0,
				      &extended);
	if (curr_p == 0)
		return -ENOSPC;

	nova_dbg_trans("%s: inode %lu attr change entry @ %#llx\n", __func__,
		       sih->ino, curr_p);
	...
	if (metadata_csum) {
		...
		nova_dbg_trans("%s: inode %lu attr change alter entry @%#llx\n",
					 ...
					 u64 last_setattr)
{
	...
	addr = (void *)nova_get_virt_addr_from_offset(sb, last_setattr, 1);
					   ...
					   u64 epoch_id)
{
	...
	if (last_log) {
		entry = (struct nova_setattr_logentry *)
			nova_get_virt_addr_from_offset(sb, last_log, 1);
					     ...
					     struct iattr *attr, u64 epoch_id)
{
	...
	return nova_inplace_update_log_entry(sb, inode, entry, &entry_info);
}
				  ...
				  struct iattr *attr, u64 epoch_id)
{
	...
	if (!(ia_valid & ATTR_SIZE) &&
		...
		nova_inplace_update_setattr_entry(sb, inode, sih, attr,
						  epoch_id);
	} else {
				      ...
				      u64 old_link_change)
{
	...
	addr = (void *)nova_get_virt_addr_from_offset(sb, old_link_change, 1);
					   ...
					   u64 epoch_id)
{
	...
	if (last_log) {
		entry = (struct nova_link_change_entry *)
			nova_get_virt_addr_from_offset(sb, last_log, 1);
		if (entry->epoch_id == epoch_id)
			return 1;
	}
				 ...
				 struct nova_inode_update *update)
{
	...
	ret = nova_append_log_entry(sb, pi, pic, inode, sih, &entry_info);
	if (ret)
		nova_err(sb, "%s failed\n", __func__);
				   ...
				   unsigned long num_pages)
{
	...
	if (prev_blocknr) {
		...
		curr_block = nova_get_block_off(sb, prev_blocknr, btype, 1);
		curr_page = (struct nova_inode_log_page *)
			nova_get_virt_addr_from_offset(sb, curr_block, 1);
		next_page = nova_get_block_off(sb, first_blocknr, btype, 1);
	...
	for (i = 0; i < num_pages - 1; i++) {
		curr_block =
			nova_get_block_off(sb, first_blocknr + i, btype, 1);
		curr_page = (struct nova_inode_log_page *)
			nova_get_virt_addr_from_offset(sb, curr_block, 1);
		next_blocknr = first_blocknr + i + 1;
		next_page = nova_get_block_off(sb, next_blocknr, btype, 1);
		nova_memunlock_block(sb, curr_page, &irq_flags);
	...
	/* Last page */
	curr_block =
		nova_get_block_off(sb, first_blocknr + num_pages - 1, btype, 1);
	curr_page =
				  ...
				  enum nova_alloc_direction from_tail)
{
	...
	allocated = nova_new_log_blocks(sb, sih, &new_inode_blocknr, num_pages,
	...
	/* Coalesce the pages */
	nova_coalesce_log_pages(sb, 0, new_inode_blocknr, sih->i_blk_type,
				allocated);
				     ...
				     int log_id)
{
	...
	allocated = nova_allocate_inode_log_pages(
		sb, sih, 1, &new_block, ANY_CPU, log_id == MAIN_LOG ? 0 : 1);
	if (allocated != 1) {
				 ...
				 struct nova_inode_info_header *sih, u64 curr_p)
{
	...
	nova_dbg_trans("%s: inode %lu, curr 0x%llx\n", __func__, sih->ino,
		       curr_p);

	if (curr_p == 0) {
		ret = nova_initialize_inode_log(sb, pi, pic, sih, MAIN_LOG);
		if (ret)
			return 0;

		if (metadata_csum) {
			ret = nova_initialize_inode_log(sb, pi, pic, sih,
							ALTER_LOG);
			if (ret)
				return 0;
	...
	num_pages = sih->log_pages >= EXTEND_THRESHOLD ? EXTEND_THRESHOLD :
							 sih->log_pages;
	nova_dbg_trans("%s: allocate %ld log pages\n", __func__, num_pages);
	allocated = nova_allocate_inode_log_pages(sb, sih, num_pages,
						  &new_block, ANY_CPU, 0);
	nova_dbg_trans("Link block %llu to block %llu\n", curr_p >> PAGE_SHIFT,
	...
	if (metadata_csum) {
		allocated = nova_allocate_inode_log_pages(
			sb, sih, num_pages, &alter_new_block, ANY_CPU, 1);
		if (allocated <= 0) {
			nova_err(sb, "%s ERROR: no inode log page available\n",
				 __func__);
			nova_dbg("curr_p 0x%llx, %#lx pages\n", curr_p,
		...
		alter_curr_page = (struct nova_inode_log_page *)
			nova_get_virt_addr_from_offset(sb, alter_curr, 1);
		nova_memunlock_block(sb, curr_page, &irq_flags);
			 ...
			 int *extended)
{
	...
	if (curr_p == 0 ||
		...
		if (thorough_gc == 0) {
			nova_dbg_trans("%s: %s, curr_p: %#llx", __func__,
				       (log_id == MAIN_LOG) ? "MAIN_LOG" :
							      "ALTER_LOG",
				       curr_p);
			curr_p =
				nova_extend_inode_log(sb, pi, pic, sih, curr_p);
			nova_dbg_trans("Extend log to %#llx\n", curr_p);
		} else {
	...
	if (is_last_entry(curr_p, size)) {
		nova_memunlock_block(
			sb, nova_get_virt_addr_from_offset(sb, curr_p, 1),
			&irq_flags);
		nova_set_next_page_flag(sb, curr_p);
		nova_memlock_block(
			sb, nova_get_virt_addr_from_offset(sb, curr_p, 1),
