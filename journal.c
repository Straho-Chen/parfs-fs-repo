...
nova_print_lite_transaction(struct nova_lite_journal_entry *entry)
{
	nova_dbg(
		...
		entry, entry->type, entry->data1, entry->data2, entry->csum);
}

static inline int
nova_update_journal_entry_csum(struct super_block *sb,
...
static inline int
nova_check_entry_integrity(struct super_block *sb,
			   struct nova_lite_journal_entry *entry)
{
	u32 crc = 0;
...
static u64 nova_append_replica_inode_journal(struct super_block *sb, u64 curr_p,
					     struct inode *inode)
{
	struct nova_lite_journal_entry *entry;
...
static u64 nova_append_entry_journal(struct super_block *sb, u64 curr_p,
				     void *field)
{
	struct nova_lite_journal_entry *entry;
...
static u64 nova_journal_inode_tail(struct super_block *sb, u64 curr_p,
...
static u64 nova_append_inode_journal(struct super_block *sb, u64 curr_p,
				     struct inode *inode, int new_inode,
				     int invalidate, int is_dir)
{
	...
	if (new_inode) {
		curr_p = nova_append_entry_journal(sb, curr_p, &pi->valid);
	} else {
		curr_p = nova_journal_inode_tail(sb, curr_p, pi);
		if (invalidate) {
			curr_p = nova_append_entry_journal(sb, curr_p,
							   &pi->valid);
			curr_p = nova_append_entry_journal(
				sb, curr_p, &pi->delete_epoch_id);
		}
	}
...
static u64 nova_append_dentry_journal(struct super_block *sb, u64 curr_p,
...
void nova_flush_journal_in_batch(struct super_block *sb, u64 head, u64 tail)
{
	void *journal_entry;
...
u64 nova_create_inode_transaction(struct super_block *sb, struct inode *inode,
				  struct inode *dir, int cpu, int new_inode,
				  int invalidate)
{
	struct journal_ptr_pair *pair;
	u64 temp;
	...
	temp = nova_append_inode_journal(sb, temp, inode, new_inode, invalidate,
					 0);

	temp = nova_append_inode_journal(sb, temp, dir, new_inode, invalidate,
					 1);
...
u64 nova_create_rename_transaction(
	struct super_block *sb, struct inode *old_inode, struct inode *old_dir,
	struct inode *new_inode, struct inode *new_dir,
	struct nova_dentry *father_entry, struct nova_dentry *new_dentry,
...
u64 nova_create_logentry_transaction(struct super_block *sb, void *entry,
				     enum nova_entry_type type, int cpu)
{
	struct journal_ptr_pair *pair;
	size_t size = 0;
	int i, count;
...
int nova_lite_journal_soft_init(struct super_block *sb)
{
	...
	sbi->journal_locks = kcalloc(sbi->cpus, sizeof(spinlock_t), GFP_KERNEL);
