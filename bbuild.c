...
static void nova_destroy_blocknode_tree(struct super_block *sb, int cpu)
{
	...
	nova_destroy_range_node_tree(sb, &free_list->data_list.block_free_tree);
	nova_destroy_range_node_tree(sb, &free_list->meta_list.block_free_tree);
}
...
static int _loop_resolve_blockmap_entry(struct super_block *sb,
					struct nova_inode_info_header *sih)
{
	...
	while (curr_p != sih->log_tail) {
		...
		if (sub_free_list->num_blocknode == 1)
			sub_free_list->first_node = blknode;
		sub_free_list->last_node = blknode;
...
static int nova_init_blockmap_from_inode(struct super_block *sb)
{
	...
	if (ret)
		goto out;
	sih.ino = NOVA_DATA_BLOCKNODE_INO;
	if (_loop_resolve_blockmap_entry(sb, &sih)) {
		nova_err(sb, "Failed to resolve blockmap entry\n");
		ret = -EINVAL;
		goto out;
	}
	...
	if (ret)
		goto out;
	sih.ino = NOVA_META_BLOCKNODE_INO;
	if (_loop_resolve_blockmap_entry(sb, &sih)) {
		nova_err(sb, "Failed to resolve blockmap entry\n");
		ret = -EINVAL;
		goto out;
...
static u64 nova_save_free_list_blocknodes(struct super_block *sb, int cpu,
					  u64 temp_tail, int meta)
{
	struct sub_free_list *sub_free_list;

	sub_free_list = nova_get_sub_free_list(sb, cpu, meta);

	temp_tail = nova_save_range_nodes_to_log(
		sb, &sub_free_list->block_free_tree, temp_tail, 0);
	return temp_tail;
}
...
void nova_save_inode_list_to_log(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		inode_map = &sbi->inode_maps[i];
		temp_tail = nova_save_range_nodes_to_log(
			sb, &inode_map->inode_inuse_tree, temp_tail, i);
	}

	nova_memunlock_inode(sb, pi, &irq_flags);
	pi->alter_log_head = pi->alter_log_tail = 0;
	pi->log_head = new_block;
	nova_update_tail(pi, temp_tail, 1);
...
void _nova_save_blocknode_mappings_to_log(struct super_block *sb,
					  struct nova_inode *pi, int meta)
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	struct nova_inode_info_header sih;
	struct sub_free_list *sub_free_list;
	...
	for (i = 0; i < sbi->cpus; i++)
		temp_tail =
			nova_save_free_list_blocknodes(sb, i, temp_tail, meta);
...
void nova_save_blocknode_mappings_to_log(struct super_block *sb)
{
	struct nova_inode *pi;
	pi = nova_get_inode_by_ino(sb, NOVA_DATA_BLOCKNODE_INO);
	_nova_save_blocknode_mappings_to_log(sb, pi, 0);
	pi = nova_get_inode_by_ino(sb, NOVA_META_BLOCKNODE_INO);
	_nova_save_blocknode_mappings_to_log(sb, pi, 1);
}

static int nova_insert_blocknode_map(struct super_block *sb, int cpuid,
				     int meta, unsigned long low,
				     unsigned long high)
{
	struct sub_free_list *sub_free_list;
	struct rb_root *tree;
				      ...
				      int meta)
{
	...
	start = sub_free_list->block_start;
	end = sub_free_list->block_end + 1;
	while (1) {
...
static void nova_update_4K_map(struct super_block *sb, struct scan_bitmap *bm,
			       unsigned long *bitmap, unsigned long bsize,
			       unsigned long scale)
{
	unsigned long next = 0;
	unsigned long low = 0;
				    ...
				    unsigned long initsize)
{
	...
	final_bm->scan_bm_4K.bitmap =
		kvzalloc(final_bm->scan_bm_4K.bitmap_size, GFP_KERNEL);
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		nova_update_4K_map(sb, bm, bm->scan_bm_2M.bitmap,
				   bm->scan_bm_2M.bitmap_size * 8,
				   PAGE_SHIFT_2M - 12);
		nova_update_4K_map(sb, bm, bm->scan_bm_1G.bitmap,
				   bm->scan_bm_1G.bitmap_size * 8,
				   PAGE_SHIFT_1G - 12);
	}
	...
	ret = __nova_build_blocknode_map(sb, final_bm->scan_bm_4K.bitmap,
					 final_bm->scan_bm_4K.bitmap_size * 8,
					 PAGE_SHIFT - 12, 0);
...
static int alloc_bm(struct super_block *sb, unsigned long initsize)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		...
		/* Alloc memory to hold the block alloc bitmap */
		bm->scan_bm_4K.bitmap =
			kvzalloc(bm->scan_bm_4K.bitmap_size, GFP_KERNEL);
		bm->scan_bm_2M.bitmap =
			kvzalloc(bm->scan_bm_2M.bitmap_size, GFP_KERNEL);
		bm->scan_bm_1G.bitmap =
			kvzalloc(bm->scan_bm_1G.bitmap_size, GFP_KERNEL);
...
static unsigned int
nova_check_old_entry(struct super_block *sb, struct nova_inode_info_header *sih,
		     u64 entry_addr, unsigned long pgoff, unsigned int num_free,
		     ...
		     struct scan_bitmap *bm)
{
	...
	// ret = nova_append_data_to_snapshot(sb, entryc, old_nvmm, num_free,
	// 				   epoch_id);
			       ...
			       struct scan_bitmap *bm)
{
	...
	if (old_entry)
		nova_check_old_entry(sb, sih, old_entry, old_pgoff, num_free,
	...
	unsigned long base, struct scan_bitmap *bm)
{
	...
	if (entryc->num_pages != entryc->invalid_pages) {
		...
		if (entryc->pgoff < base + MAX_PGOFF &&
			...
			nova_set_ring_array(sb, sih, entry, entryc, ring, base,
					    bm);
	}
...
static void
nova_clear_file_entry_data_blocks(struct super_block *sb,
				  struct nova_inode_info_header *sih,
				  struct nova_file_write_entry *entry,
				  struct nova_file_write_entry *entryc,
				  struct task_ring *ring, unsigned long base)
{
	...
	for (pgoff = entryc->pgoff; pgoff < entryc->pgoff + entryc->num_pages;
	     pgoff++) {
		index = pgoff - base;
		ring->nvmm_array[index] = 0;
		ring->entry_array[index] = 0;
	...
	entry->invalid_pages = entryc->num_pages;
	nova_flush_buffer(&entry->invalid_pages, sizeof(u32), 0);
}
...
again:
	...
	while (true) {
		...
		case LINK_CHANGE:
			...
			break;
		case FILE_WRITE:
			if (WENTRY(entryc)->trans_id != sih->trans_id) {
		...
		case MMAP_WRITE:
			...
			break;
		default:
			nova_dbg_verbose("End of log entry\n");
			loop_end = true;
			break;
		}
		if (loop_end)
	...
	if (curr_p != pi->log_tail) {
		nova_dbg_verbose("%s: pi tail corrupt\n", __func__);
		// restore tail
		pi->log_tail = curr_p;
		nova_update_tail(pi, curr_p, 1);
	}

	if (sih->ckpt_id != sih->trans_id) {
		nova_dbg_verbose("%s: check latest trans: %llu\n", __func__, sih->trans_id);
		// check last trans data blocks
		u64 tmp = trans_curr;
		bool invalid = false;
		loop_end = false;
		nova_dbg_verbose("%s: trans_curr: %#llx, curr: %#llx\n", __func__, trans_curr, curr_p);
		while (tmp != curr_p) {
			if (goto_next_page(sb, tmp)) {
				tmp = next_log_page(sb, tmp);
			}
			entry = (void *)nova_get_virt_addr_from_offset(sb, tmp,
								       1);
			type = nova_get_entry_type(entry);
			switch (type) {
			case SET_ATTR:
				tmp += sizeof(struct nova_setattr_logentry);
				break;
			case LINK_CHANGE:
				tmp += sizeof(struct nova_link_change_entry);
				break;
			case FILE_WRITE:
				if (!nova_vaild_data_csum(sb, sih, entry)) {
					// csum corrupt, roll back whole transaction
					nova_dbg_verbose(
						"%s: data csum corrupt, roll back whole transaction\n",
						__func__);
					invalid = true;
					loop_end = true;
					break;
				...
				break;
			case MMAP_WRITE:
				tmp += sizeof(struct nova_mmap_entry);
				break;
			default:
				// nova_dbg_verbose("\n");
				loop_end = true;
		...
		if (invalid) {
			nova_dbg_verbose("%s: inode %lu is invalid\n", __func__,
					 sih->ino);
			pi->recovery_latest_trans_invalid = 1;
			nova_flush_buffer(&pi->recovery_latest_trans_invalid,
					  sizeof(u32), 0);
			tmp = trans_curr;
			loop_end = false;
			while (tmp != curr_p) {
				...
				case LINK_CHANGE:
					...
					break;
				case FILE_WRITE:
					// free data blocks, remove from nvmm_array is enough
					nova_clear_file_entry_data_blocks(
						sb, sih, entry, entry, ring,
						base);
					tmp += sizeof(
...
static int allocate_resources(struct super_block *sb, int cpus)
{
	...
	for (i = 0; i < cpus; i++) {
		threads[i] = kthread_create(failure_thread_func, sb,
					    "recovery thread");
		...
		for (i = 0; i < num_inodes_per_page; i++) {
			...
			if (fake_pi.i_mode && fake_pi.deleted == 0) {
				...
				nova_recover_inode_pages(sb, &sih, ring,
							 &fake_pi,
							 global_bm[cpuid]);
				nova_failure_update_inodetree(sb, pi, &ino_low,
							      &ino_high);
...
static int nova_failure_recovery_crawl(struct super_block *sb)
{
	...
	ret = nova_get_reference(sb, root_addr, &fake_pi, (void **)&pi,
				 sizeof(struct nova_inode), 1);
	if (ret) {
	...
	nova_recover_inode_pages(sb, &sih, &task_rings[0], &fake_pi,
				 global_bm[0]);
...
static bool nova_try_normal_recovery(struct super_block *sb)
{
	...
	if (ret) {
		nova_err(
			sb,
			"init blockmap failed, fall back to failure recovery\n");
		return false;
	}
	...
	if (ret) {
		nova_err(
			sb,
			"init inode list failed, fall back to failure recovery\n");
		nova_destroy_blocknode_trees(sb);
		return false;
	}
	...
	if (sbi->mount_snapshot == 0) {
		...
		if (ret) {
			nova_err(
				sb,
				"Restore snapshot table failed, fall back to failure recovery\n");
...
int nova_recovery(struct super_block *sb)
{
	...
	unsigned long meta_size = le64_to_cpu(super->s_meta_size);
	unsigned long data_size = le64_to_cpu(super->s_data_size);
	bool value = false;
	int ret = 0;
	INIT_TIMING(start);
	INIT_TIMING(end);
