...
/* raid5 functions */
static u64 nova_block_parity_call(char **data, char *parity, size_t size,
				  int disks)
{
...
static u64 nova_block_csum_parity_call(char **data, char *parity, size_t size,
				       int disks)
{
	int i;
	size_t strp_size = size;
	...
	for (i = 0; i < strp_size / 8; i++) {
		...
		// if (data_parity > 0) {
		parity[i] = qwd[0] ^ qwd[1] ^ qwd[2] ^ qwd[3] ^ qwd[4] ^
			    qwd[5] ^ qwd[6] ^ qwd[7];
		// }
...
static void *nova_alloc_pmem_pool(struct super_block *sb,
				  struct nova_inode_info_header *sih, int cpu,
				  size_t poolsize, unsigned long *blocknr,
				  int *allocated)
{
	int num;
	void *pool;
		   ...
		   unsigned int poolmb, size_t size, unsigned int disks)
{
	...
	if (func_id == 0) {
		...
		for (id = 1; id <= NUM_PERF_CALLS; id++) {
			ret = nova_test_func_perf(sb, id, poolsize, size,
						  disks);
			if (ret < 0)
				goto out;
		}
