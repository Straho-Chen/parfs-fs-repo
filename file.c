...
/* Do integrity checking on recovery. */
	...
	while (num_blocks > 0) {
		...
		// we do data csum on optfs cow write, no need is_dele check
		/* calculate data checksum and write csum to pmem */
		NOVA_START_META_TIMING(bd_data_csum_t, bd_data_csum_time);
#if NOVA_KERNEL_COPY_USER_BUFFER
		ret = nova_protect_file_data(sb, inode, pos, bytes, ubuf_copy,
					     blocknr);
#else
