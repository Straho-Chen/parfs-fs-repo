...
static int nova_check_dentry_match(struct super_block *sb,
				   struct nova_dentry *dentry, const char *name,
				   int namelen)
{
...
static unsigned int nova_init_dentry(struct super_block *sb,
				     struct nova_dentry *de_entry, u64 self_ino,
...
static int nova_inplace_update_dentry(struct super_block *sb, struct inode *dir,
				      struct nova_dentry *dentry,
				      int link_change, u64 epoch_id)
{
	...
	return nova_inplace_update_log_entry(sb, dir, dentry, &entry_info);
}
		       ...
		       bool rename)
{
	...
	ret = nova_append_dentry(sb, pidir, dir, dentry, 0, loglen, update,
				 dec_link, epoch_id);
...
static int nova_readdir_slow_rbtree(struct file *file, struct dir_context *ctx)
{
	...
	if (pos == 0)
		...
		found = nova_find_range_node(&sih->rb_tree, pos, NODE_DIR,
					     &curr);
...
static int nova_readdir_fast(struct file *file, struct dir_context *ctx)
{
	...
	while (curr_p != sih->log_tail) {
		...
		if (entryc->ino > 0 && entryc->invalid == 0 &&
			...
			if (ret) {
				nova_dbg(
					"%s: get child inode %lu address failed %d\n",
					__func__, ino, ret);
				ctx->pos = READDIR_END;
				return ret;
			}
	...
	if (prev_entry &&
	    !dir_emit(ctx, prev_entryc->name, prev_entryc->name_len, ino,
