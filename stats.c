...
static void nova_print_alloc_stats(struct super_block *sb)
{
	...
	nova_info(
		"Fast GC %llu, check pages %llu, free pages %llu, average %llu\n",
		Countstats[fast_gc_t], IOstats[fast_checked_pages],
		IOstats[fast_gc_pages],
		Countstats[fast_gc_t] ?
			IOstats[fast_gc_pages] / Countstats[fast_gc_t] :
			0);
	nova_info(
		"Thorough GC %llu, checked pages %llu, free pages %llu, average %llu\n",
		Countstats[thorough_gc_t], IOstats[thorough_checked_pages],
		IOstats[thorough_gc_pages],
...
static void nova_print_IO_stats(struct super_block *sb)
{
	...
	nova_info("Read %llu, bytes %llu, average %llu\n",
		  ...
		  Countstats[dax_read_t] ?
			  IOstats[read_bytes] / Countstats[dax_read_t] :
			  0);
	nova_info(
		"COW write %llu, bytes %llu, average %llu, write breaks %llu, average %llu\n",
		Countstats[do_cow_write_t], IOstats[cow_write_bytes],
		Countstats[do_cow_write_t] ?
			IOstats[cow_write_bytes] / Countstats[do_cow_write_t] :
			0,
		IOstats[cow_write_breaks],
		Countstats[do_cow_write_t] ?
			IOstats[cow_write_breaks] / Countstats[do_cow_write_t] :
			0);
	nova_info(
		"Inplace write %llu, bytes %llu, average %llu, write breaks %llu, average %llu\n",
...
static inline void
nova_print_file_write_entry(struct super_block *sb, u64 curr,
			    struct nova_file_write_entry *entry)
{
	nova_dbg(
		"file write entry @ 0x%llx: epoch %llu, trans %llu, pgoff %llu, pages %u, blocknr %llu, reassigned %u, updating %u, invalid count %u, size %llu, mtime %u\n",
		curr, entry->epoch_id, entry->trans_id, entry->pgoff,
		entry->num_pages, entry->block >> PAGE_SHIFT, entry->reassigned,
		entry->updating, entry->invalid_pages, entry->size,
		entry->mtime);
}

static inline void
nova_print_set_attr_entry(struct super_block *sb, u64 curr,
			  struct nova_setattr_logentry *entry)
{
	nova_dbg(
		"set attr entry @ 0x%llx: epoch %llu, trans %llu, invalid %u, mode %u, size %llu, atime %u, mtime %u, ctime %u\n",
		curr, entry->epoch_id, entry->trans_id, entry->invalid,
		entry->mode, entry->size, entry->atime, entry->mtime,
		entry->ctime);
}

static inline void
nova_print_link_change_entry(struct super_block *sb, u64 curr,
			     struct nova_link_change_entry *entry)
{
	nova_dbg(
		"link change entry @ 0x%llx: epoch %llu, trans %llu, invalid %u, links %u, flags %u, ctime %u\n",
		curr, entry->epoch_id, entry->trans_id, entry->invalid,
		entry->links, entry->flags, entry->ctime);
}

static inline void nova_print_mmap_entry(struct super_block *sb, u64 curr,
					 struct nova_mmap_entry *entry)
{
	nova_dbg(
		"mmap write entry @ 0x%llx: epoch %llu, invalid %u, pgoff %llu, pages %llu\n",
		curr, entry->epoch_id, entry->invalid, entry->pgoff,
		entry->num_pages);
}

static inline void
nova_print_snapshot_info_entry(struct super_block *sb, u64 curr,
			       struct nova_snapshot_info_entry *entry)
{
	nova_dbg(
		"snapshot info entry @ 0x%llx: epoch %llu, deleted %u, timestamp %llu\n",
		curr, entry->epoch_id, entry->deleted, entry->timestamp);
}

static inline size_t nova_print_dentry(struct super_block *sb, u64 curr,
				       struct nova_dentry *entry)
{
	nova_dbg(
		"dir logentry @ 0x%llx: epoch %llu, trans %llu, reassigned %u, invalid %u, inode %llu, links %u, namelen %u, rec len %u, name %s, mtime %u\n",
		curr, entry->epoch_id, entry->trans_id, entry->reassigned,
		entry->invalid, le64_to_cpu(entry->ino), entry->links_count,
		entry->name_len, le16_to_cpu(entry->de_len), entry->name,
		entry->mtime);
...
int nova_check_inode_logs(struct super_block *sb, struct nova_inode *pi)
{
	...
	while (curr && alter_curr) {
		if (alter_log_page(sb, curr) != alter_curr ||
			...
			nova_dbg(
				"Inode %llu page %d: curr 0x%llx, alter 0x%llx, alter_curr 0x%llx, alter 0x%llx\n",
				pi->nova_ino, count1, curr,
				alter_log_page(sb, curr), alter_curr,
...
void nova_print_free_lists(struct super_block *sb)
{
	...
	for (i = 0; i < sbi->cpus; i++) {
		for (j = 0; j < sbi->sockets; j++) {
			free_list = nova_get_free_list(sb, i, j);
			nova_dbg(
				"Free list cpu%d,socket%d: block start %lu, block end %lu, num_blocks %lu, num_free_blocks %lu, blocknode %lu\n",
				i, j, free_list->block_start,
				free_list->block_end,
			...
			nova_dbg(
				"Free list cpu%d,socket%d: csum start %lu, replica csum start %lu, csum blocks %lu, parity start %lu, parity blocks %lu\n",
				i, j, free_list->csum_start,
			...
			nova_dbg(
				"Free list cpu%d,socket%d: alloc log count %lu, allocated log pages %lu, alloc data count %lu, allocated data pages %lu, free log count %lu, freed log pages %lu, free data count %lu, freed data pages %lu\n",
				i, j, free_list->alloc_log_count,
