				...
				struct nova_dentry **res_entry)
{
	...
	direntry = nova_find_dentry(sb, NULL, dir, entry->name, entry->len);
	if (direntry == NULL)
		return 0;
...
static void nova_lite_transaction_for_new_inode(
	struct super_block *sb, struct nova_inode *pi, struct nova_inode *pidir,
 ...
 * with d_instantiate().
 */
static int nova_create(struct mnt_idmap *idmap, struct inode *dir,
...
static int nova_mknod(struct mnt_idmap *idmap, struct inode *dir,
		      struct dentry *dentry, umode_t mode, dev_t rdev)
{
	...
	inode = nova_new_vfs_inode(idmap, TYPE_MKNOD, dir, pi_addr, ino, mode,
...
static int nova_symlink(struct mnt_idmap *idmap, struct inode *dir,
			struct dentry *dentry, const char *symname)
{
	...
	inode = nova_new_vfs_inode(idmap, TYPE_SYMLINK, dir, pi_addr, ino,
				   S_IFLNK | 0777, len, 0, &dentry->d_name,
				   epoch_id);
	if (IS_ERR(inode)) {
		err = PTR_ERR(inode);
		goto out_fail;
...
static void nova_lite_transaction_for_time_and_link(
	struct super_block *sb, struct nova_inode *pi, struct nova_inode *pidir,
	struct inode *inode, struct inode *dir,
	struct nova_inode_update *update, struct nova_inode_update *update_dir,
	int invalidate, u64 epoch_id)
{
	struct nova_sb_info *sbi = NOVA_SB(sb);
	u64 journal_tail;
	int cpu;
	...
	// If you change what's required to create a new inode, you need to
	// update this functions so the changes will be roll back on failure.
	journal_tail = nova_create_inode_transaction(sb, inode, dir, cpu, 0,
						     invalidate);
		     ...
		     struct dentry *dentry)
{
	...
	update.tail = 0;
	update.alter_tail = 0;
	err = nova_append_link_change_entry(sb, pi, inode, &update, &old_linkc,
					    epoch_id);
	if (err) {
		iput(inode);
		goto out;
	...
	d_instantiate(dentry, inode);
	nova_lite_transaction_for_time_and_link(
		sb, pi, pidir, inode, dir, &update, &update_dir, 0, epoch_id);
...
static int nova_mkdir(struct mnt_idmap *idmap, struct inode *dir,
		      struct dentry *dentry, umode_t mode)
{
	...
	inode = nova_new_vfs_inode(idmap, TYPE_MKDIR, dir, pi_addr, ino,
				   S_IFDIR | mode, sb->s_blocksize, 0,
...
static int nova_rmdir(struct inode *dir, struct dentry *dentry)
{
	...
	nova_lite_transaction_for_time_and_link(
		sb, pi, pidir, inode, dir, &update, &update_dir, 1, epoch_id);
...
static int nova_rename(struct mnt_idmap *idmap, struct inode *old_dir,
		       ...
		       struct dentry *new_dentry, unsigned int flags)
{
	...
	err = nova_add_dentry(new_dentry, old_inode->i_ino, inc_link,
			      &update_dir_new, epoch_id);
	if (err)
		goto out;
	...
	journal_tail = nova_create_rename_transaction(
		sb, old_inode, old_dir, new_inode,
		old_dir != new_dir ? new_dir : NULL, father_entry,
		new_inode ? update_dir_new.create_dentry : NULL,
		update_dir_old.create_dentry, invalidate_new_inode, cpu);
	if (new_inode)
