...
inline int nova_search_inodetree(struct nova_sb_info *sbi, unsigned long ino,
				 struct nova_range_node **ret_node)
{
	...
	return nova_find_range_node(tree, internal_ino, NODE_INODE, ret_node);
}
			   ...
			   u64 *pi_addr, int extendable, int extend_alternate)
{
		...
		if (curr == 0) {
			...
			*(u64 *)(curr_addr) = curr;
			nova_memlock_range(sb, (void *)curr_addr,
					   CACHELINE_SIZE, &irq_flags);
			nova_flush_buffer((void *)curr_addr, NOVA_INODE_SIZE,
					  1);
		}
	}
	...
	if (extended && extend_alternate && metadata_csum)
		nova_get_inode_address(sb, ino, version + 1, &alternate_pi_addr,
				       extendable, 0);
				 ...
				 u64 *alter_pi_addr)
{
	...
	if (metadata_csum == 0) {
		nova_err(sb,
			 "Access alter inode when replica inode disabled\n");
		return 0;
	}
			  ...
			  bool delete_dead, u64 epoch_id)
{
	...
	if (old_entry && delete_nvmm) {
		nova_free_old_entry(sb, sih, old_entry, old_pgoff, num_free,
				    delete_dead, epoch_id);
		freed += num_free;
	}
				      ...
				      loff_t end, u64 epoch_id)
{
	...
	freed = nova_delete_file_tree(sb, sih, first_blocknr, last_blocknr,
				      true, false, epoch_id);
		...
		if (entry) {
			...
			*hole_found = 1;
			...
			if (entry) {
				...
				pgoff = pgoff > entryc->pgoff ? pgoff :
								entryc->pgoff;
...
void nova_evict_inode(struct inode *inode)
{
	...
	if (pi && pi->nova_ino != inode->i_ino) {
		...
		nova_dbg(
			"inode size %llu, pi addr 0x%lx, pi head 0x%llx, tail 0x%llx, mode %u\n",
...
struct inode *nova_new_vfs_inode(struct mnt_idmap *idmap,
				 enum nova_new_inode_type type,
...
int nova_getattr(struct mnt_idmap *idmap, const struct path *path,
...
int nova_notify_change(struct mnt_idmap *idmap, struct dentry *dentry,
		       struct iattr *attr)
{
	...
	ret = nova_handle_setattr_operation(sb, inode, pi, ia_valid, attr,
					    epoch_id);
	if (ret)
		goto out;
...
unsigned long nova_find_region(struct inode *inode, loff_t *offset, int hole)
{
	...
	nova_dbg_verbose(
		"find_region offset %llx, first_blocknr %lx, last_blocknr %lx hole %d\n",
		*offset, first_blocknr, last_blocknr, hole);

	blocks = nova_lookup_hole_in_range(inode->i_sb, sih, first_blocknr,
