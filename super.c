...
MODULE_PARM_DESC(wprotect,
		 "Write-protect pmem region and use CR0.WP to allow updates");
...
MODULE_PARM_DESC(dram_struct_csum,
			      ...
			      bool remount)
{
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_wprotect:
			...
			nova_info(
				"NOVA: Enabling new Write Protection (CR0.WP)\n");
			break;
		case Opt_dbgmask:
			if (match_int(&args[0], &option))
				goto bad_val;
...
static int nova_check_integrity(struct super_block *sb)
{
	...
	if (rc < 0) {
		...
		if (rc < 0) {
			...
			nova_warn(
				"Error in super block: try to repair it with the other copy\n");
	}
...
static int nova_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (sbi->cpus > MAX_CPUS) {
		nova_err(
			sb,
			"NOVA needs more log pointer pages to support more than " __stringify(
				MAX_CPUS) " cpus.\n");
		goto out;
	}
	...
	sbi->inode_maps =
		kcalloc(sbi->cpus, sizeof(struct inode_map), GFP_KERNEL);
	if (!sbi->inode_maps) {
		retval = -ENOMEM;
		nova_dbg("%s: Allocating inode maps failed.", __func__);
		goto out;
	}
	...
	if (!sbi->zeroed_page) {
		retval = -ENOMEM;
		nova_dbg("%s: sbi->zeroed_page failed.", __func__);
		goto out;
	}
	...
	if (!sbi->zero_parity) {
		retval = -ENOMEM;
		nova_err(sb, "%s: sbi->zero_parity failed.", __func__);
		goto out;
	}
	...
	if (sbi->s_mount_opt & NOVA_MOUNT_FORMAT) {
		...
		retval = -ENOMEM;
		if (IS_ERR(root_pi)) {
			nova_err(sb, "%s: root_pi error.", __func__);
...
setup_sb:
	...
	if ((sbi->s_mount_opt & NOVA_MOUNT_FORMAT) == 0) {
		...
		if (retval < 0) {
			nova_err(
				sb,
				"%s: nova recovery failed with return code %d\n",
				__func__, retval);
			goto out;
		}
	}
	...
	if (IS_ERR(root_i)) {
		...
		nova_err(sb, "%s: failed to get root inode", __func__);
...
struct snapshot_info *nova_alloc_snapshot_info(struct super_block *sb)
{
	...
	p = (struct snapshot_info *)kmem_cache_alloc(nova_snapshot_info_cachep,
						     GFP_NOFS);
	return p;
}
...
struct nova_range_node *nova_alloc_range_node_atomic(struct super_block *sb)
{
	...
	p = (struct nova_range_node *)kmem_cache_zalloc(nova_range_node_cachep,
							GFP_ATOMIC);
	return p;
}
...
static int __init init_rangenode_cache(void)
{
	nova_range_node_cachep = kmem_cache_create(
		"nova_range_node_cache", sizeof(struct nova_range_node), 0,
...
static int __init init_snapshot_info_cache(void)
{
	nova_snapshot_info_cachep = kmem_cache_create(
		"nova_snapshot_info_cache", sizeof(struct snapshot_info), 0,
		(SLAB_RECLAIM_ACCOUNT | SLAB_MEM_SPREAD), NULL);
...
{
	nova_inode_cachep = kmem_cache_create(
		"nova_inode_cache", sizeof(struct nova_inode_info), 0,
...
static struct dentry *nova_mount(struct file_system_type *fs_type, int flags,
...
static struct inode *nova_nfs_get_inode(struct super_block *sb, u64 ino,
					u32 generation)
{
	struct inode *inode;
...
static struct dentry *nova_fh_to_dentry(struct super_block *sb, struct fid *fid,
...
static struct dentry *nova_fh_to_parent(struct super_block *sb, struct fid *fid,
					int fh_len, int fh_type)
{
